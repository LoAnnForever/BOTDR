...
#define IOCTL_GET_BUF_INFO              XDMA_IOCTL(0x8)
#define IOCTL_MAP_EVT                   XDMA_IOCTL(0x9)
#define IOCTL_MAP_ADDR                  XDMA_IOCTL(0xa)
#define IOCTL_WRITE_CREDITS             XDMA_IOCTL(0xb)
#define IOCTL_UNMAP_ADDR                XDMA_IOCTL(0xc)
//constants for DMA data buffer
#define XDMA_RING_NUM_BLOCKS    (8)
#define XDMA_RING_BLOCK_SIZE    (32*1024UL*1024UL)//(8*1024UL*1024UL)//(PAGE_SIZE)
#define XDMA_MAX_TRANSFER_SIZE  (XDMA_RING_NUM_BLOCKS*XDMA_RING_BLOCK_SIZE)//(16UL * 1024UL * 1024UL)
...
typedef struct _DmaDataBufAddr
{
    PVOID UserAddr;
    ULONG PhyAddr_LowPart;
    ULONG PhyAddr_HighPart;
    ULONG ByteCount;
}DmaDataBufAddr, *PDmaDataBufAddr;

typedef struct
{
  UINT head;//start pointer
  UINT nexthead;//new head to give back to hw
